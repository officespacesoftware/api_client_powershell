...
# User config
###############
$ossHostname = "xxxxxxxx.officespacesoftware.com"                  # OfficeSpace instance hostname
...
ForEach ($employee in $employees) {
    $employee | Add-Member -Name site -Value "" -MemberType NoteProperty #streetAddress
    $employee | Add-Member -Name floor -Value "" -MemberType NoteProperty #extensionAttribute13
    ...
    if ($UpdateAD) {
        ...
        $ad_user = $null
        $ad_user = Get-ADUser -Filter "employeeId -eq '$($employee.client_employee_id)'" -Properties physicalDeliveryOfficeName, streetAddress, extensionAttribute13
   
        write-host "UPDATING $($employee.client_employee_id): $($employee | Select-Object site, floor, siteseat)"

        # Commented out the logic that clears the attributes in AD if the seat in OfficeSpace is empty.
        #if (($employee.siteseat -eq "")) {
        #    Set-ADUser $ad_user -Clear streetAddress, extensionAttribute13, physicalDeliveryofficeName
        #}
        #
        #else { 
            # Added a clause to only update the attributes in AD if the seats differ AND the siteseat attribute is not empty.
            if (($ad_user.physicalDeliveryOfficeName -ne $employee.siteseat) -and  ($employee.siteseat -ne "")) {
                Set-ADUser $ad_user -Replace @{physicalDeliveryofficeName = $employee.siteseat; extensionAttribute13 = $employee.floor; streetAddress = $employee.site }
            }
        #}
