...
# User config
###############
$ossHostname = "xxxxxxxxxx.officespacesoftware.com"             # OfficeSpace instance hostname
...
ForEach ($employee in $employees) {
    ...
    ForEach ($seatURL in $employee.online_seating.seat_urls) {
        ...
        $employee.location += "$($seatsMap.get_item($seatURL).label),"
    }
    $employee.floor = $employee.floor.TrimEnd(",");
    $employee.site = $employee.site.TrimEnd(",");
    $employee.location = $employee.location.TrimEnd(",");
    ...
    if ($UpdateAD) {
        ...
        write-host "UPDATING $($employee.client_employee_id): $($employee | Select-Object location)"

        if (($employee.location -eq "")) {
            if (($ad_user.physicalDeliveryOfficeName) -ne ($employee.location)) {
            ...
            if (($ad_user.physicalDeliveryOfficeName) -ne ($employee.location)) {
                Set-ADUser $ad_user -Replace @{physicalDeliveryofficeName = $employee.location}
            }
        }
...
if ($OutputCSV) {
    $employees | Select-Object client_employee_id, site, floor, location | Export-Csv -Delimiter $outputDelimiter -Path "$($outputPath)\$($outputFile)" -NoTypeInformation
}